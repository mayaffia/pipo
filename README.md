...
Проект использует GitHub Actions для автоматической сборки и тестирования при каждом коммите в ветку `main`.

### Процесс CI/CD

1. **Backend Tests** - Запуск линтера и тестов для backend с PostgreSQL
2. **Frontend Tests** - Запуск линтера и тестов для frontend
3. **Build** - Сборка Docker образов (только для main ветки)

### Статус сборки

Статус последней сборки отображается в бейдже вверху README. При ошибке сборки:

- На GitHub появляется красная плашка ❌
- В разделе Actions можно посмотреть детали ошибки

### Что проверяется при каждом коммите

- Линтинг кода (ESLint)
- Unit и интеграционные тесты
- Сборка TypeScript проектов
